...
for audit_folder in audit_folders:
    ...
    if not audit_files:
        ...
        for i in range(len(audit_files)):
            ...
            set_date = datetime.datetime.strptime('24-04-01', '%y-%m-%d')
            
            #date of the audit file
            file_date = audit_files[i].split(audit_folder)[1].split(' -')[1][-6:]
            file_date_formated = str(file_date[0:2]+'-'+file_date[2:4]+'-'+file_date[4:6])
            try:                
                file_date = datetime.datetime.strptime(file_date_formated, '%y-%m-%d')
            except:
                continue
            ...
            if (file_date > set_date) or (file_date == set_date):
                ...
                #                     row_detail.append(str(value.replace("'","")))
                #                 else:
