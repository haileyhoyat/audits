...
mdm_audit_header = 'mdm_audit_header.csv'
mdm_audit_detail = 'mdm_audit_detail.csv'
...
for audit_folder in audit_folders:
    ...
    if not audit_files:
        ...
        for i in range(len(audit_files)):
            ...
            file_date = audit_files[i].split(audit_folder)[1].split('.xlsx')[0][-15:-9:]
            file_date_formated = str(file_date[0:2]+'-'+file_date[2:4]+'-'+file_date[4:6])
            #file time
            file_time = audit_files[i].split(audit_folder)[1].split('.xlsx')[0][-6::]
